...
import { LuMessagesSquare, LuUser } from 'react-icons/lu';
...
const RenderSidebar = ({ currentTab, setCurrentTab, MAIN_COLOR }) => {
  ...
  const NoChat = () => (
    <div className="flex flex-col items-center mt-24 h-full">
      ...
      <div className="text-center flex flex-col text-gray-500">
        <span className="text-xs">{t('no_chats_yet')}</span>
        {user?.role === 'student' && (
          <span className="text-xs">{t('start_by_messaging_teachers')}</span>
        )}
      </div>
    </div>
  );
  ...
  return (
    <div className="p-2 lg:p-4 h-full">
      <h2 className={`font-bold text-lg text-[${MAIN_COLOR}]`}>{t('messages')}</h2>
        ...
        {isFetchingChats ? (
          ...
          chats
            ...
            .map((chat) => {
              ...
              const typingUsers = (sidebarTyping[chat.id] || []).filter((id) => id !== user?.id);
