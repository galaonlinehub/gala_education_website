...
import { getFooterLinks } from '@/utils/data/links';
      ...
      <div className="max-w-screen-2xl mx-auto h-full justify-start flex lg:items-center flex-col lg:flex-row lg:px-16 lg:justify-between w-full">
        <div className="text-white lg:w-[30rem] md:w-[40rem] items-center justify-center flex-col">
          ...
          <div className="text-xs">
            <h2>{footer('be_the_first')}</h2>
            <h2>{footer('sign_up_now')}</h2>
          </div>
          <form onSubmit={handleSubmit(onSubmit)} className="pt-4 flex flex-col gap-2 w-full">
            {status.message && (
              <div
                className={clsx(
                  'text-xs border-[0.1px] p-2 w-full rounded-md mb-2 text-center',
                  status.type === 'success' && ' border-green-600 text-green-600',
            ...
            <div className="flex w-full gap-4">
              <div className="flex flex-col w-[75%]">
                ...
                />
                {errors.email && (
                  <span className="text-red-500 text-[10px] p-1">{errors.email.message}</span>
                )}
              </div>
              <button
                disabled={
                  emailSubscriptionMutation.isPending || emailSubscriptionMutation.isSuccess
                }
                type="submit"
                className={clsx(
        ...
        <div className="flex flex-col pt-10 pb-6 gap-y-12">
              ...
              <div key={i}>
                ...
                <ul>
                  {links.map((linkItem, idx) => (
                    <li className="text-white text-xs" key={idx}>
                      {linkItem.name}
                    </li>
                  ))}
                </ul>
          ...
          <div>
            <p className="text-white text-xs">
              © {currentYear} {footer('all_rights_reserved_1')} <br />{' '}
              {footer('all_rights_reserved_2')} <br /> {footer('all_rights_reserved_3')}
            </p>
          </div>
        </div>
      </div>
