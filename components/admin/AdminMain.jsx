    ...
    <div className='flex w-screen'>
        ...
        <div className='py-2'>
      {
                    adminLinks.map(({name,link,icon:Icon},i)=>(
                        <Link onClick={()=>setOpen(false)} key={i} href={link} className={`flex  items-center justify-between px-4 py-2 w-full ${pathname === link ? "bg-blue-900 text-white font-black" : "text-gray-600"} `}>
                            <h1>{name}</h1>
                            <Icon />
        ...
        <div className='flex justify-center items-center w-full absolute bottom-10 left-0 '>
          <MdClose className='text-red bg-gray-300 h-10 w-10 rounded-full p-2' onClick={()=>setOpen(false)} />
        </div>
      </Drawer>
    </div>
  )
