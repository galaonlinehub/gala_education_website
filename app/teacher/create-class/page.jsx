...
export default function CreateClass() {
  ...
  const {
    ...
    queryKey: ["grades"],
    queryFn: async () => {
      const response = await fetch("https://galaweb.galahub.org/api/grade_levels");
      ...
      <div className="max-w-2xl mx-auto p-6 border-2 border-blue-700 shadow-md relative">
        ...
        <form onSubmit={handleSubmit} className="space-y-4 text-xs">
          ...
          <div className="mb-4">
            ...
            <select name="subject" value={formData.subject} onChange={handleChange} onBlur={handleBlur} className={`w-full border-2 ${touched.subject && errors.subject ? "border-red-500" : "border-blue-700"} p-2`} disabled={isLoading || isRedirecting}>
          ...
          <div className="mb-4">
            ...
            {touched.availableTo && errors.availableTo && <p className="mt-1 text-xs text-red-500">{errors.availableTo}</p>}
          </div>

          <div className={`mb-4 ${formData.availableTo && formData.subject ? "block" : "hidden"}`}>
            <label className="block mb-2 font-bold">Topic</label>
            <select name="topic" value={formData.topic} onChange={handleChange} onBlur={handleBlur} className={`w-full border-2 ${touched.topic && errors.topic ? "border-red-500" : "border-blue-700"} p-2`} disabled={isLoading || isRedirecting}>
            ...
            {touched.topic && errors.topic && <p className="mt-1 text-xs text-red-500">{errors.topic}</p>}
          </div>

          <div className="mb-4">
            <label className="block mb-2 font-bold">Enter Price</label>
            <input type="number" name="price" value={formData.price} onChange={handleChange} onBlur={handleBlur} placeholder="Enter price" className={`w-full border-2 ${touched.price && errors.price ? "border-red-500" : "border-blue-700"} p-2`} disabled={isLoading || isRedirecting} />
          ...
          <div className="mb-4">
            ...
            <input type="date" name="startDate" value={formData.startDate} onChange={handleChange} onBlur={handleBlur} className={`w-full border-2 ${touched.startDate && errors.startDate ? "border-red-500" : "border-blue-700"} p-2`} disabled={isLoading || isRedirecting} />
          ...
          <div className="mb-4">
            ...
            <input type="date" name="endDate" value={formData.endDate} onChange={handleChange} onBlur={handleBlur} className={`w-full border-2 ${touched.endDate && errors.endDate ? "border-red-500" : "border-blue-700"} p-2`} disabled={isLoading || isRedirecting} />
            {touched.endDate && errors.endDate && <p className="mt-1 text-xs text-red-500">{errors.endDate}</p>}
          </div>

          <div className="mb-4">
            <label className="block mb-2 font-bold">Description</label>
            <textarea name="description" value={formData.description} onChange={handleChange} onBlur={handleBlur} rows="4" placeholder="Enter class description" className={`w-full border-2 ${touched.description && errors.description ? "border-red-500" : "border-blue-700"} p-2`} disabled={isLoading || isRedirecting} />
