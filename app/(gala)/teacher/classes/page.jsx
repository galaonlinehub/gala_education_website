...
export default function TeacherClasses() {
  ...
  return (
    <div className="relative flex flex-col lg:flex-row justify-between p-4 lg:p-5">
      <div className="flex flex-col w-full lg:w-2/3">
        <div className="p-4 h-fit mt-20 bg-orange-200 w-full border-blue-600 border-2 rounded-xl flex flex-col relative">
          <div>
        ...
        <div className="pt-5">
          <h2 className="text-xs font-bold mb-2">Your Classes</h2>
        ...
        <div className="pt-5 mb-20">
          <span className="text-xs font-bold">Your Lessons</span>
          <div className="flex flex-col gap-3">
            <div className="bg-blue-950 w-full flex flex-wrap justify-between items-center font-bold text-xs text-white rounded-md p-2">
              {["Class", "Members", "Starting", "End", "Material", "Payment"].map((header, index) => (
                <span key={index} className="w-1/2 sm:w-1/3 md:w-1/6 text-center py-2">
                  {header}
                </span>
              ))}
            </div>
            {["1", "IV"].map((subject, index) => (
              <div key={index} className="border-2 border-blue-600 flex flex-wrap items-center justify-between font-bold text-xs rounded-md w-full p-2">
                <span className="w-1/2 sm:w-1/3 md:w-1/6 text-center py-2">{subject}</span>
                <span className="w-1/2 sm:w-1/3 md:w-1/6 text-center py-2">10</span>
                <span className="w-1/2 sm:w-1/3 md:w-1/6 text-center py-2">10.05.2026</span>
                <span className="w-1/2 sm:w-1/3 md:w-1/6 text-center py-2">10.06.2026</span>
                <span className="w-1/2 sm:w-1/3 md:w-1/6 text-center py-2">View</span>
                <span className="w-1/2 sm:w-1/3 md:w-1/6 text-center py-2">{index === 0 ? "Done" : "Pending"}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <button onClick={() => setShowSidebar(!showSidebar)} className="fixed bottom-4 right-4 lg:hidden bg-blue-900 text-white p-2 rounded-full z-10">
        {showSidebar ? "Hide" : "Show"} Sidebar
      </button>

      <div className={`bg-blue-900 rounded-xl w-full lg:w-80 mt-10 items-center flex flex-col justify-center h-full ${showSidebar ? "fixed inset-0 z-50" : "hidden lg:flex"}`}>
        <div style={wrapperStyle} className="m-6 bg-gray-50">
          <Calendar fullscreen={false} onPanelChange={onPanelChange} />
        </div>
        <hr className="bg-white w-5/6" style={{ height: "0.4px", border: "none" }} />
