...
const StudentSocial = () => {
  ...
  const renderContent = () => {
    ...
    return filteredUsers.map((item, index) => (
      <div key={index} onClick={() => viewOnClickedUser(index)} className={`p-2 mb-3 shadow-md flex cursor-pointer gap-2 rounded-lg ${clickedUserIndex === index ? "bg-[#001840] text-white" : "bg-white"}`}>
  ...
  const renderChat = () => (
    <div className="flex flex-col h-full">
      <div className="bg-gray-200 p-4 text-black font-bold flex items-center">
        {isSmallScreen && (
        ...
        <h2 className="text-xs">{users[clickedUserIndex].name}</h2>
      </div>
      <div className="flex-grow overflow-y-auto p-4 space-y-2">
        {messages.map((message, index) => (
          <div key={index} className={`p-2 rounded-lg ${message.sender === "You" ? "bg-[#070B65] ml-auto" : "bg-[#8E8C90]"} max-w-[70%]`}>
                ...
                <div className="w-full md:w-1/3 p-4 border-r border-gray-300">
                  ...
                  <div className="flex justify-between gap-1 mb-4">
                    {["Direct", "Groups", "Public"].map((tab) => (
                      <button key={tab} onClick={() => setCurrentTab(tab.toLowerCase())} className={`px-2 text-sm py-1 rounded ${currentTab === tab.toLowerCase() ? "bg-blue-500 text-white" : "bg-gray-200"}`}>
                        {tab}
                      </button>
                    ))}
                  </div>
        ...
        {(!isSmallScreen || clickedUserIndex === null) && (
          <div className="w-full lg:w-1/3">
            <div className="bg-[#001840] rounded-xl p-6">
              <h2 className="text-white text-sm font-bold mb-4">Notifications</h2>
              <div className="space-y-4 mb-8">
                {notifications.map((item, index) => (
                  <div key={index} className="bg-white rounded-md p-2 flex items-start gap-3">
                    <Image className="rounded-full" src="/necklace.png" alt="avatar" width={32} height={32} />

                    <div>
                      <p className="text-blue-600 text-xs font-bold">{item.name}</p>
                      <p className="text-xs font-bold">{item.head}</p>
                      <p className="text-xs truncate max-w-[150px]">{item.msg}</p>
                    </div>
                  </div>
                ))}
              </div>
