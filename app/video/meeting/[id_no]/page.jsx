...
//             mainVideoRef.current.appendChild(newElem);
//           }
...
//         if (element) mainVideoRef.current.removeChild(element);
//       }
//     });
...
//               <div id="videoContainer" ref={mainVideoRef}></div>
//             </td>
//           </tr>
//         </tbody>
//       </table>
...
const VideoConference = ({ params: { id_no: roomId } }) => {
  ...
  // Rest of the existing mediasoup implementation...
    ...
    const streamSuccess = (stream) => {
      ...
      if (role === "student" && mainVideoRef.current) {
        const newElem = document.createElement("div");
        newElem.setAttribute("id", `td-local-student`);
        newElem.setAttribute("data-username", userName);
        newElem.className = "remoteVideo";
    
        newElem.innerHTML = `
          <div>
            <video id="localStudentVideo" autoplay muted class="video"></video>
            <p>${userName} (${role})</p>
          </div>
        `;
        ...
        // Set the local stream to the new video element
        const localStudentVideo = document.getElementById("localStudentVideo");
        if (localStudentVideo) {
          localStudentVideo.srcObject = stream;
        }
      }
    ...
    <div ref={containerRef} className="flex w-screen gap-x-3 max-h-screen overflow-auto p-4 bg-[#efefeb]">
      ...
      <div className="basis-1/3 p-4">
        {sectionPage === "participants" && (
          <div ref={participantsRef} id="remoteVideo" className="grid grid-cols-2 gap-4">
            {Object.entries(participantStreams).map(([id, { stream, user }]) => (
              <div key={id} className="relative w-full h-40 rounded-md overflow-hidden">
                <video
                  autoPlay
                  playsInline
