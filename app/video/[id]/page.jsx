...
const VideoConference = ({ params: { id: roomId } }) => {
  ...
  const initializeDevice = async () => {
    try {
      ...
      // Get router RTP capabilities with room context
      const rtpCapabilities = await new Promise((resolve, reject) => {
        socketRef.current.emit("getRouterRtpCapabilities", { roomId }, (response) => {
          if (response.error) {
      ...
      await setupTransports(mediasoupDevice);
    } catch (err) {
      ...
      const response = await new Promise((resolve, reject) => {
        socketRef.current.emit("joinRoom", { roomId }, (response) => {
          if (response.error) {
            ...
            setRoomParticipants(response.participants);
            // Handle existing producers
            
            console.log("inside of here room objectis ",response)
            if (response.producers) {
              response.producers.forEach(({ producerId, producerSocketId }) => {
                consumeMedia(producerId, producerSocketId);
              });
            }
            resolve(response);
          }
  ...
  const setupTransports = async (mediasoupDevice) => {
    try {
      const sendTransportOptions = await new Promise((resolve, reject) => {
        socketRef.current.emit("createSendTransport", { roomId }, (response) => {
          if (response.error) reject(new Error(response.error));
          else resolve(response);
        });
      });

      const sendTransport = mediasoupDevice.createSendTransport(sendTransportOptions);
      ...
      transports.current.send = sendTransport;
      ...
      setError(`Failed to setup transport: ${err.message}`);
    }
  };

  const consumeMedia = async (producerId, producerSocketId) => {
    try {
      if (!device?.loaded) {
        throw new Error("Device not loaded");
      }

      if (producerSocketId === socketRef.current.id) {
        return;
      }

      if (!transports.current.receive) {
        const transportOptions = await new Promise((resolve, reject) => {
          socketRef.current.emit("createRecvTransport", { roomId }, (response) => {
            if (response.error) reject(new Error(response.error));
            else resolve(response);
          });
        });
        ...
        recvTransport.on("connect", ({ dtlsParameters }, callback, errback) => {
          ...
          transports.current.receive = recvTransport;
        }
        ...
        const consumer = await transports.current.receive.consume({
        ...
        // Update remote streams state with the new stream
        ...
        setError(`Failed to consume media: ${err.message}`);
      }
    };
    ...
    const initializeSocket = () => {
      try {
        ...
        socket.on("participantLeft", ({ participantId }) => {
          ...
          // Clean up remote streams for the participant who left
          setRemoteStreams(prev => {
            const updated = { ...prev };
          ...
          // Clean up consumers for the participant who left
        ...
        socket.on("newProducer", ({ producerId, producerSocketId }) => {
          console.log("New producer detected:", producerId, "from", producerSocketId);
    ...
    const cleanup = () => {
      ...
      Object.values(transports.current).forEach(transport => {
      ...
      <div className="flex flex-col min-h-screen p-4 bg-gray-100">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          ...
          {Object.entries(remoteStreams).map(([participantId, stream]) => (
            <div key={participantId} className="relative bg-black rounded-lg overflow-hidden aspect-video shadow-lg">
              <video
                autoPlay
                playsInline
                className="w-full h-full object-cover"
              ...
              />
              <div className="absolute bottom-2 left-2 bg-black bg-opacity-50 text-white px-2 py-1 rounded">
                {roomParticipants.find(p => p.id === participantId)?.user?.name || 'Remote User'}
              </div>
            </div>
          ))}
        </div>
