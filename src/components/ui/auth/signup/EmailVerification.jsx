        ...
        <div>
            ...
            {Array(6)
              ...
              .map((_, index) => (
                <input
                  ...
                  className={clsx(" text-3xl font-black w-12 h-12 text-center text-black rounded-md focus:outline-none focus:ring",
                    verifyMutate.isSuccess
                      ...
                      : verifyMutate?.error?.status === 429 ?
                        ...
                        : verifyMutate?.error ? "border border-red-500 focus:ring-red-500 focus:outline-red-500 input-shake failure"
          ...
          )}
          {
            (verifyMutate.isSuccess ? (
              ...
              verifyMutate?.error?.status === 429 ? <div className="flex flex-col items-center justify-center gap-2">
                ...
                <span>{verifyMutate.error?.response?.data?.message || verifyMutate.error?.message}</span>
              </div>
            ) : "")}
          <div className="flex flex-wrap gap-2 text-xs w-full items-center justify-end overflow-hidden">
            ...
            <Button
              ...
              className={clsx("!p-1 !bg-transparent !rounded-md !cursor-pointer !text-xs disabled:pointer-events-none", handleResendMutation.isPending && "!text-blue-600 !px-1")
              }
              disabled={resendCounter > 0 || verifyMutate.isPending || handleResendMutation.isPending}
