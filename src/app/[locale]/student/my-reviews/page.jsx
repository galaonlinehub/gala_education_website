...
import { useStudentReviews } from "@/hooks/data/useStudentReviews";
import { apiPut } from "@/services/api/api_service";
...
const UserReviewsInterface = () => {
  ...
  const {
    studentInstructorReviews,
    studentCohortReviews,
    refetchInstructorReviews,
    refetchCohortReviews,
  } = useStudentReviews(cohort_id, instructor_id);
      ...
      {enrolledTopics?.length > 0 ? (
        enrolledTopics?.map((item) => (
          <Col xs={24} lg={12} key={item.id}>
            <Card className="rounded-xl shadow-sm border-l-4 border-l-[#001840] !cursor-pointer">
              ...
              <div className="flex gap-1 items-center w-full justify-between">
                ...
                <Button
                  type="link"
                  onClick={() => fetchInstructorReviews(item.instructor_id)}
                  className="p-0 h-auto !text-[#001840] hover:text-blue-800 text-xs sm:text-sm font-semibold"
                >
                  View Reviews{" "}
          ...
          <Col xs={24} lg={12} key={item.id}>
            <Card className="rounded-xl shadow-sm border-l-4 border-l-[#001840] !cursor-pointer">
              ...
              <div className="flex gap-1 items-center w-full justify-between">
                ...
                <Button
                  type="link"
                  onClick={() => fetchCohortReviews(item.cohort_id)}
                  className="p-0 h-auto !text-[#001840] hover:text-blue-800 text-xs sm:text-sm font-semibold"
                >
                  View Reviews{" "}
                  <IoChevronForwardCircle
  ...
  return (
    <div className="p-6 min-h-screen">
      <div className="max-w-7xl mx-auto">
        <div>
          <div className="text-center mb-6">
            ...
            <Text className="!text-xs sm:!text-sm" type="secondary">
              View and update your evaluations
            </Text>
          </div>
            ...
            <div className="border-b border-gray-200 mb-6">
              <nav className="flex w-full">
                ...
                >
                  ...
                  <span className="sm:hidden !text-xs sm:!text-sm">
                    Instructor
                  </span>
                  <Badge
      ...
      >
        {editingReview && (
          <div className="mt-4">
            <Text className="!text-xs sm:!text-sm" strong>
              Teacher:{" "}
              <span className="text-blue-600">
                {editingReview.reviewable_name}
              </span>
            </Text>
